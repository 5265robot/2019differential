...
/**
 * First stab at coding JeVois
 * Will copy over code one piece at a time for understanding.
 * from team 1736
 */
public class vision extends Subsystem {
     ...
     */
     ...
     public vision(boolean useUSBstream){
          ...
          while (visionPort == null && counter++ < 10) {
               try {
                    ...
                    e.printStackTrace();
                    sleep(500);         //sleep is a private method
                    System.out.println("Retry " + Integer.toString(counter));
               }
          }
          ...
          if(sendPing() != 0){          //sendPing is a private method
          ...
          stopDataOnlyStream();         //stopDataOnlyStream is a private method
          setCameraStreamActive(useUSBstream);
          start();                      //start is a public method
     ...
     public void start(){        
          if(broadcastUSBCam){            
               ...
               startCameraStream();     //startCameraStream is a private method
          } else {            
               startDataOnlyStream();        
          }    
     }
                         ...
                         if(packetBuffer.indexOf(PACKET_START_CHAR) != -1){                    	
                              ...
                              if(endIdx != -1){                            
                                   ...
                                   if(startIdx == -1){                                
                                        ...
                                        value = packetBuffer.substring(startIdx+1, endIdx-1);                                
     ...
     */
     private int sendPing() {
          int value = -1;
          if (visionPort != null){
               value = sendCmdAndCheck("ping");    //sendCmdAndCheck is a private method
          }
          return value;
     }
     ...
     */
     public int sendCmdAndCheck(String cmd){
          int value = 0;
          sendCmd(cmd);                           //sendCmd is a private method
          value = blockAndCheckForOK(1.0);        //blockAndCheckForOK is a private method
          if(value == -1){
     ...
     private void stopCameraStream(){
          ...
          if(camStreamRunning){            
               camServer.free();        // deprecated -- use 'close()'??
               visionCam.free();           
